...
* SourceTree 이외에도 여러가지 클라이언트 앱이 많은데, 그 중에 더 간단한 것으로는 Github Desktop이 있고, 더 예쁜(?) 것으로는 Git Kraken이 있다.  Git Kraken의 경우에는 LFS 기능이 Beta 딱지를 붙이고 활성화되어 있는데, 살짝 동작의 신뢰성이 떨어져 보인다. (리눅스 버전으로 사용해 보니 일부 동작이 제대로 안 되는 것 같다.)  Github Desktop의 경우에는 UI가 굉장히 단순화되어 있어 초보자에게 좀 더 적합해 보인다.  다만 LFS 기능이 제대로 되는지는 확인이 필요하다.
